...
/** Rubros válidos para poblar selects y para las filas agregables de la visita. Es la
 *  MISMA lista contra la que el backend valida la propuesta (RubroCatalogService) — no
 *  confundir con /clients/getRubros, que es otra query sobre staging, sin cache y con
 *  filtros propios. Ordenada por descripción del lado del server.
 *
 *  No se deriva de `getRubroStatus`: ese es el historial del cliente, y un cliente sin
 *  movimientos no tiene ninguno (ver spec 2026-09-16-rubros-agregables-desde-el-catalogo). */
